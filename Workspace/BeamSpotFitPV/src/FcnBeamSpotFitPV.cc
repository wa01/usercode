...
void
FcnBeamSpotFitPV::getBins (TAxis* axis, std::vector<double>& centres) const
{
  unsigned int nbin = axis->GetNbins();
  centres.resize(nbin);
  for ( unsigned int i=1; i<=nbin; ++i ) {
    centres[i-1] = axis->GetBinCenter(i);
  }
}

void
FcnBeamSpotFitPV::fillPdfs (TH3* hPos, TH3* hPdf, TH1* hChi2,
			    const std::vector<float>& pars) const
{

  std::vector<double> binCentresX;
  getBins(hPdf->GetXaxis(),binCentresX);
  std::vector<double> binCentresY;
  getBins(hPdf->GetYaxis(),binCentresY);
  std::vector<double> binCentresZ;
  getBins(hPdf->GetZaxis(),binCentresZ);
  double binWidth = hPdf->GetXaxis()->GetBinWidth(1)*
    hPdf->GetYaxis()->GetBinWidth(1)*hPdf->GetZaxis()->GetBinWidth(1);
  //
  // fit parameters
  //
  double vb1 = pars[0];
...
// parametrisation: rotation (dx/dz, dy/dz); covxy
  ...
  for ( vector<BeamSpotFitPVData>::const_iterator ipv=data_.begin();
    ...
    //
    // filling of 3d histograms
    //
    double det;
    cov.Det2(det);
    double scale = binWidth/sqrt(det*2.*3.14159265)/(2.*3.14159265);
    for ( unsigned int ix=0; ix<binCentresX.size(); ++ix ) {
      dv(0) = binCentresX[ix] - vb1;
      for ( unsigned int iy=0; iy<binCentresY.size(); ++iy ) {
	dv(1) = binCentresY[iy] - vb2;
	for ( unsigned int iz=0; iz<binCentresZ.size(); ++iz ) {
	  dv(2) = binCentresZ[iz] - vb3;
	  double expo = ROOT::Math::Similarity(dv,wgt);
	  double pdf = expo<30. ? exp(-expo/2.) : 0.;
 	  hPdf->Fill(binCentresX[ix],binCentresY[iy],binCentresZ[iz],scale*pdf);
	}
      }
    }
    // chi2
    ...
    dv(1) = v2 - vb2;
    dv(2) = v3 - vb3;
    double chi2 = ROOT::Math::Similarity(dv,wgt);
    if ( chi2<0 )  chi2 = 0;
    double chi2prob = TMath::Prob(chi2,3);
    hChi2->Fill(chi2prob);
  }
