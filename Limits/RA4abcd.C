...
//   int nbx = hBkg->GetNbinsX();
//   int nby = hBkg->GetNbinsY();
//   for ( unsigned int ix0=1; ix0<=nbx; ix0+=dHT ) {
//     for ( unsigned int ix1=ix0+dHT; ix1<=nbx; ix1+=dHT ) {
//       for ( unsigned int ix2=ix1; ix2<=nbx; ix2+=dHT ) {
// 	for ( unsigned int iy0=1; iy0<=nby; iy0+=dMET ) {
// 	  for ( unsigned int iy1=iy0+dMET; iy1<=nby; iy1+=dMET ) {
// 	    for ( unsigned int iy2=iy1; iy2<=nby; iy2+=dMET ) {
