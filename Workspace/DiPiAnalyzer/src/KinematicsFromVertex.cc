#include "Workspace/DiPiAnalyzer/interface/KinematicsFromVertex.h"
#include "FWCore/MessageLogger/interface/MessageLogger.h"


KinematicsFromVertex::LorentzVector KinematicsFromVertex::p4 (const CachingVertex<5>& vertex,
...
GlobalVector KinematicsFromVertex::momentum(const CachingVertex<5>& vertex) const
{
...
Measurement1D KinematicsFromVertex::invariantMass(const CachingVertex<5>& vertex,
                          const double mass) const
{
...
double KinematicsFromVertex::uncertainty(const LorentzVector & totalP4, 
	const CachingVertex<5>& vertex, const std::vector<double> & masses) const
{
  ...
  for( ;rt_i !=refTracks.end(), i_m != masses.end(); ++rt_i, ++i_m) {
    ...
    if ((**rt_i).linearizedTrack()->charge()!=0) {
      ...
      if (a==0.) throw cms::Exception("KinematicsFromVertex", "Field is 0");
    } else {
      a = 1;
    }
