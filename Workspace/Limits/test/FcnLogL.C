...
public:
//   FcnLogL() {}
  FcnLogL(const std::vector<Triplet>& triplets, int nTot) : theTriplets(triplets), nTot_(nTot), theErrorDef(0.5) {}
  // ~FcnLogL() {delete thePVLogL;}
  double Up() const {return theErrorDef;}
  double operator()(const std::vector<double>&) const;
private:
  double logBinomial (double x, double par) const;
  double logL (const Triplet& triplet, const std::vector<double>& pars) const;
private:
  std::vector<Triplet> theTriplets;
  int nTot_;
  double theErrorDef;
};
#endif //MN_FcnLogL_H_
...
double
FcnLogL::logL (const Triplet& triplet, const std::vector<double>& pars) const
{
  double x = triplet.x();
  double y = triplet.y();
  double z = triplet.z();
